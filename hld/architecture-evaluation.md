...
| SP-9 | Number of remote sites with line of sight (A17) | Cost of reach, and the number of delivery modes | At three or more no-line-of-sight sites the pickup collector is bought and a third delivery mode is operated; at one it is not, and those sites take cellular. The site survey moves the estate between a two-channel and a three-channel design, and nothing else does |
...
| TP-11 | Three delivery channels for remote sites instead of one | Cameras at enclosures that would otherwise have none, without a permanent per-site bill | A third delivery mode to operate, a route dependency, and a missed-round failure mode — paid so that the critical class never depends on a vehicle | [ADR-0019](../adrs/ADR-0019-reach-for-remote-enclosures.md), R21 |
