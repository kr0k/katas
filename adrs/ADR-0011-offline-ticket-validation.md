...
## Decision
1. Tickets are **signed credentials** (QR/NFC payload with ticket id, validity window, pass type, group size, a slot reservation for a capacity-managed day if any, signature). Gates verify the **signature offline** with a public key — no lookup needed to admit a valid, unused ticket.
2. The **local gate validation service** keeps a synced allow-list/revocation list (sold, refunded, already used) and a local "used" ledger.
   - Lists arrive over the **downlink** as retained snapshots plus sequence-numbered deltas in the critical traffic class, so a reconnecting gate holds current revocations within a minute ([downlink](../hld/core/edge-and-connectivity.md#downlink-cloud--estate)).
   - Admission requires a valid signature and absence from the local used-ledger.
   - Each admission records `GateEntered` to the local broker **with `persons_admitted`**: the people let through on this scan — 1 for a single ticket, the group-counter increment for a pass, **0 for a same-day re-entry**.
   - Visitor-days across this repository are Σ `persons_admitted` ([requirements/08 §0](../requirements/08-business-case.md#0-how-to-read-the-numbers)), reconciled daily against admissions sold. A vendor whose gate events carry only the credential fails the acceptance test ([ADR-0012](ADR-0012-ticketing-platform-adopt-not-build.md) §2).
...
| **Exit and re-entry** the same day | An exit scan marks the credential "out"; re-entry within the operational day is admitted without incrementing the counter and with `persons_admitted = 0`, so nobody is counted twice; a re-entry attempt with no exit scan is treated as a possible duplicate → staff override with a reason code, not a refusal |
| **Capacity-managed day** — a pass holder arrives without a reservation (FR-1.7) | The reservation is an attribute of the signed credential and is checked offline like the signature; without one the pass is admitted only while the day's admissions are below the cap in the downlink snapshot; over it, refused with a pointer to the next free slot and the desk — the cap never touches ticket holders, who bought a slot |
