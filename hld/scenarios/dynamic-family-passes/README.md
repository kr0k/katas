...
**Moves:** OKR 1.5 (weekday/weekend ratio 0.35 → 0.38 base / 0.5 stretch → 0.6 — the primary objective), 1.1 (visitors/day), 1.4 (revenue per identified household per year — must not fall)
...
Pricing's job in the [flywheel](../../../requirements/08-business-case.md#3-the-membership-flywheel) is the second turn: a season pass priced below two visits, and a same-day upgrade that credits today's ticket, so that the family that liked Wednesday comes back in March. The quiet-day experiment fills the calendar; the pass fills the year. The two are kept apart on purpose: **S5 prices day and family tickets for specific quiet dates**; a **season pass is not date-specific**, so its price is management's yearly decision (like the base price) and the same-day upgrade credit is a ticketing-platform invariant — **P-I7** in [ADR-0012](../../../adrs/ADR-0012-ticketing-platform-adopt-not-build.md), executed by the vendor. S5 references P-I7 and never prices a pass. Weekday fill by pass holders is S4's nudges; S5 fills weekdays with day-ticket buyers.
...
- **Weekly guardrail:** OKR 1.4 is an annual figure and cannot steer a weekly experiment, so the operational guardrail is **contribution per visitor-day on discounted blocks not below the 0% blocks − 10%**; a discount level that breaks it is paused for the next block ([thresholds table](../../ai-platform/README.md#thresholds-and-cadences-source-of-truth)).
...
- **Business:** OKR 1.5 measured monthly against the fixed-price counterfactual; OKR 1.4 (revenue per identified household per year) yearly — the weekly guardrail is its operational proxy ("OKR = year, guardrail = week", [06](../../../requirements/06-suggested-okrs.md)).
