...
- **Knowledge base:** structured facts (animals, rides, opening hours, safety rules, prices) plus curated narrative content; the *only* source the companion may cite → [ADR-0010](../../adrs/ADR-0010-grounded-llm-with-guardrails.md). It has a **staff tier** alongside the visitor one: the estate's approved **protocol set** — husbandry, quarantine, bite and escape procedures — versioned, owned by the vet and the head keeper, and carrying an approval state, because a protocol mid-revision must read as absent rather than current (FR-3.9). These documents exist already for regulatory reasons; the platform indexes them, and never authors one.
- **Metric layer:** the estate's **defined** business numbers — attendance, visitor-days, zone load, spend per visitor-day, repeat share, welfare review latency — each with **one owner and one formula**, versioned in Git. Everything that quotes a number reads it from here: the [daily report](#estate-daily-report), the dashboards, [`ask-the-estate`](../ai-platform/agents.md#ask-the-estate), and the forecasting and pricing models. It exists because a report and a dashboard computing "attendance" two different ways is not a reporting bug but a trust failure, and because a natural-language question resolved to a *named metric* either resolves or is refused, where text-to-SQL over raw schemas invents a join and therefore invents a number. Other proposals call this a digital twin; we add no component — it is a projection of data we already hold.
...
- **Resilience validation:** twenty-five scripted game days with expected behaviour, metric, cadence and owner → [resilience-validation.md](resilience-validation.md).
