...
| | New capability | A ninth AI use case is added — three were, during this merge | A new consumer on the backbone; no producer changes; no shared schema; the risk class decides its controls before it ships | (M, L) | [ADR-0004](../adrs/ADR-0004-event-driven-backbone.md) §3, [ADR-0017](../adrs/ADR-0017-ai-risk-classes-and-proportional-controls.md) |
...
| | Agent wiring | An agent starts proposing badly, while its model's own metrics stay green | Task success below target for two cycles → **agent rollback** of prompt, whitelist and step budget, without touching the model bundle | (H, M) | [ADR-0013](../adrs/ADR-0013-stakeholder-agents-on-typed-tools.md) §8, OKR 5.4 |
| **Agent safety** | Indirect prompt injection | A curated knowledge-base page is edited to instruct the agent to issue a free pass | Content enters as quoted data and never as the tool-selection prompt; no tool in the whitelist can issue a credential; the attempt lands as a tool-error span | (H, M) | GD-17, [trust boundary](ai-platform/agents.md#trust-boundary-with-the-chains-worked-through) |
| | Memory poisoning | A crafted session writes a false derivative that would silently shape later decisions | Typed derivatives only, through a write-guard with provenance; welfare, safety and money derivatives are human-approved; a poisoned batch is revertible by provenance | (H, H) | GD-18, [ADR-0014](../adrs/ADR-0014-two-tier-agent-memory-with-a-write-guard.md) |
| | Agent on a safety path | An agent proposes something that would bypass a rule or a certification | No agent tool sits on a tier-0 path; AI issues no clearance; the ride registry accepts a status change from one writer, enforced by a contract test | (H, L) | [ADR-0017](../adrs/ADR-0017-ai-risk-classes-and-proportional-controls.md) §4, [ADR-0022](../adrs/ADR-0022-ride-condition-monitoring.md) §3 |
| **Privacy under personalisation** | Identity added to a system designed without it | The estate issues a visitor token and still has to answer "where do families go" without tracking anyone | Anonymous counters remain the instrument of record; taps aggregated and suppressed below 20 per cell before analytics; location is a separate, refusable consent; carry rate published with the figure | (H, H) | [ADR-0018](../adrs/ADR-0018-visitor-token-and-anonymised-paths.md), GD-20, NFR-PRV-3 |
| | Erasure with more places to hide | A token holder with taps, a purchase and a companion session asks to be erased | Crypto-shredding plus `SubjectErased` through read models, features, **both agent memory tiers**, aggregated flows and golden sets; audit query returns zero within 7 days | (H, M) | GD-11, GD-20 |
| **Reach** | A remote enclosure | An enclosure beyond LoRaWAN and Wi-Fi loses both its radio bridge and a pickup round | Critical class still on its own cellular link and the keeper is paged; clips buffered for three rounds; freshness reported as degraded rather than silently stale | (H, M) | GD-19, [ADR-0019](../adrs/ADR-0019-reach-for-remote-enclosures.md) |
...
| SP-3 | Cacheable share of the session prefix | Cost-efficiency | Prompt caching is ≈ half the generative bill: €34,800 with it, €67,800 without. A prompt refactor that breaks prefix stability is a cost incident |
...
| SP-8 | Token carry rate (A16) | Usefulness of every path figure, **and** the flywheel's reach | At 40% carriage the weighting to counter totals is credible and FR-2.10 produces answers; below 15% a zone reports "not representative" and the investment question goes back to before-and-after. The same number decides how many returning households can be recognised at all, so one assumption carries both the analytics and a growth lever |
| SP-9 | Cellular coverage at the remote sites (A17) | Cost of reach, and the number of delivery modes | Coverage, not site count, decides: where it carries the classes a site costs one €400 subscription and nothing else; where it is absent the site needs a bridge, and where it is thin with no line of sight the collector and a third delivery mode arrive. The survey moves the estate between a one-, two- and three-channel design, and nothing else does — the cost arithmetic does not, which [ADR-0019](../adrs/ADR-0019-reach-for-remote-enclosures.md) §4 states rather than hides |
| SP-10 | Steps per agent task, and the tier each step runs on | Cost of the agentic layer, and its latency | The layer is €508 a year because the companion's tool-selection turn is a small-tier routing call at 2.5 steps per staff task. Route that turn to the large tier, or let step budgets drift upward, and the cheapest-looking part of the proposal becomes a visible line — which is why a self-test fails the build above 5% of generative spend |
| SP-11 | Instrumented ride count (A19) | Whether S8 exists at all | Half the forty is a capability; ten is a pilot; below that the sensors cost more than the threshold rules save, and the honest answer is to keep the statutory schedule alone |
...
| TP-8 | Four agents rather than one, or none | Least privilege that means something — the companion cannot hold the vet's tools — and a process-shaped fit for each stakeholder | Four tool inventories, eval suites, budgets and audit surfaces on a team of five; this is the proposal's largest operability cost | [ADR-0013](../adrs/ADR-0013-stakeholder-agents-on-typed-tools.md), R25 |
| TP-9 | Issuing a visitor token | A credential that survives a dead battery, recognition of a returning household without an app, and the first answer to "what did this investment change" | A visible identity surface to explain, a weighting procedure to maintain, issuance and hygiene at the gate, and the simplest possible privacy story traded for a more defensible one | [ADR-0018](../adrs/ADR-0018-visitor-token-and-anonymised-paths.md), R23 |
| TP-10 | Instrumenting the rides | The estate's first data on how its machinery behaves, and closures the forecast can see coming | A new sensor class across forty heritage assets, partial coverage that makes the metric per-ride, and a capability worth nothing for a full season | [ADR-0022](../adrs/ADR-0022-ride-condition-monitoring.md), A19 |
| TP-11 | Up to three delivery channels for remote sites instead of one | Cameras at enclosures no single channel reaches, and a bridge's latency where a metered link would not carry the clip class | A second and possibly third delivery mode to operate, a route dependency and a missed-round failure mode — and no cost saving to show for it, since cellular stays at every site that has coverage | [ADR-0019](../adrs/ADR-0019-reach-for-remote-enclosures.md) §4, R21 |
| TP-12 | Every capability ships as a rule first and keeps it | Benefit in year one, an honest promotion bar, and fallbacks proven in production rather than designed | Two implementations of every capability, permanently — and the possibility that a model never earns promotion, which we treat as a result | [ADR-0023](../adrs/ADR-0023-rules-first-model-second.md) |
...
- **Breadth is the operability risk, and it is the one we took knowingly.** Eight scenarios, four agents and twenty-three applications on five engineers (R25). What holds it is reuse — one camera pipeline, one forecast, one review queue, one metric layer — plus phase gates that can stop after two agents and four capabilities that carry an explicit kill gate. A judge who reads this as over-scoped is reading the same fact we are.
- **The token's carry rate carries both the analytics and a growth lever** (SP-8). One unmeasured number decides whether path figures mean anything and how many returning households can be recognised, and only the first season measures it.
- **Condition monitoring produces nothing for a season** and could be read as a safety claim in the meantime (R24, TP-10). The structural answer is that one writer changes a ride's status; the cultural one takes longer.

**Where a judge should push hardest.** SP-8 and SP-10: the token's carry rate, because two separate arguments rest on it, and the agentic layer's step budget, because the case for four agents is that they are nearly free and that is true only at 2.5 steps on the tier we chose.

**Non-risks**, analysed and closed:

- **Throughput.** ≈ 21 msg/s, ≈ 1.0 GB/day, ≈ 0.1 Mbps average uplink. Nothing in the estate is throughput-bound, which is why no part of the design pays for scale-out.
