...
2. **ADR-0011 is the specification, not a build plan.** Its signed-credential, local-ledger, reconcile-later design is the **acceptance test** a vendor must pass. Selection criteria, all must-have:
   ...
   | **Timed-entry slots with a daily capacity cap**, settable by ops through an API that returns the effective cap and the sold count; on **capacity-managed days a free timed-slot reservation for pass holders**, carried as an attribute of the signed credential so gates validate it offline, with unreserved passes admitted only below the cap — *to be confirmed by the vendor evaluation* | FR-1.7; the [capacity check](../requirements/08-business-case.md#1-capacity-reality-check) — parking and lunch seating bind in year 1, and a cap on tickets alone does not reach pass holders |
   | **POS / F&B included, or a separate POS** with per-transaction webhooks or export carrying the FR-2.6 fields (transaction id, type, net + tax + currency, category, terminal, time, optional pseudonymous visitor id) — *to be confirmed* | FR-2.6, OKR 1.4, A13; the [Estate daily report](../hld/core/README.md#estate-daily-report) |
   | **Upgrade credit voucher** — a day ticket scanned in today can be turned, once per ticket id, into a voucher for its price, redeemable against a season pass within 7 days; the pass starts on the visit date; a refund voids an unredeemed voucher and a ticket is not refundable once its voucher is redeemed. This is the platform invariant **P-I7**: a pass has one price for everyone; the voucher is a rule the vendor executes, not a personal price — *to be confirmed* | Second lever of the [flywheel](../requirements/08-business-case.md#3-the-membership-flywheel); S5 only references it |
   ...
   | **Vendor sandbox** covering webhooks (including replay and backlog), the cap and reservation API and the upgrade voucher — *to be confirmed* | Integration tests and the staged rollout in [hld/core → Verification](../hld/core/README.md#verification-purchases-cap-and-the-daily-report) |
   | `GateEntered` carries **`persons_admitted`** — the number of people admitted on the scan, 0 on a same-day re-entry — and the credential type — *to be confirmed* | Visitor-days are Σ `persons_admitted` ([requirements/08 §0](../requirements/08-business-case.md#0-how-to-read-the-numbers), [ADR-0011](ADR-0011-offline-ticket-validation.md) §2); the pass share of admissions comes from the credential type |
