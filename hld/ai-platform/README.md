...
| **S1 protocol answers** — `answer-protocol` (Phase 2) | Verbatim fidelity of actionable steps 100% and paraphrased steps 0 on a 100-question set; citation present 100%; **refusal 100% on 30 questions with no approved document**, including 5 whose document is pending re-approval; false refusals ≤ 5% on 100 covered questions | Any answer shown without a citation → capability off until fixed; a keeper reporting a stale procedure → incident, and the document's owner is notified | Weekly: 10 answers checked against the source by the head keeper; the protocol set re-approved on change | Vet · Head keeper |
| **S1 feed forecast** — `forecast-feed-consumption` (Phase 2) | Rolling-origin MAPE per store **below the trailing four-week average** it replaces; invariant suite on the ordering proposal (no negative orders, no order exceeding storage) violations = 0 | Stockouts > 0 in a quarter → back to the trailing average until re-qualified; MAPE above the baseline for 3 consecutive weeks → alert the owner | Keeper approves every order; monthly forecast-versus-actual per store | Head keeper |
| **S7 investment evaluation** — `estimate-investment-effect` (Phase 3) | Placebo tests: significant effects on no-investment windows ≤ the rate the interval implies; the closed-zone back-test recovers direction and rough magnitude; invariant suite (no post-period leakage, no zone as its own control) violations = 0 | Not a production guardrail but a reporting rule: an effect surviving only one control specification is labelled **fragile**, and a failed pre-trend test disqualifies the control rather than widening the interval | Quarterly readout to management with the counterfactual and assumptions shown | Management |
...
| **`agent:ops-copilot`** (Phase 2) | Task success ≥ 0.70 on a 100-task labelled set; tool-error rate ≤ 0.05; 0 instructions followed from retrieved content on the 100-case injection set; 0 duplicate effects under forced retry; step budget respected in every task; **p95 within the NFR-PRF-1 agent-task budget** | Task success below target for two weekly cycles → **agent** rollback (prompt, whitelist, budget), separate from the model bundle; tool-error rate rising → integration triage before any retraining; `open_review` items rejected by the vet > 50% → the tool is rate-limited harder or withdrawn. **Tasks per week is tracked as a first-class number**: task success and override rate both *improve* when a capability is abandoned, so falling usage is the only signal that reads an unused tool correctly | Weekly: 20 tasks labelled by the ops manager and the guest team | AI platform |
...
| **Metric layer** — the defined numbers themselves | Every metric computed against a **fixture event stream whose totals are known by construction**, violations = 0; and the same fixture replayed through each consumer — daily report, dashboards, `ask-the-estate`, the forecasting and pricing feature sets — so two consumers disagreeing about one metric fails the build. Same pattern as the daily report's read model in [business data health](../../appendix/data-health-and-verification.md#verification-purchases-cap-and-the-daily-report) | A metric whose definition changes without its owner's approval → blocked in review, since the definitions are GitOps policy config; a consumer quoting a value without its definition → capability off until fixed | Monthly: 10 answers checked against the read model by the ops manager | Ops manager |
...
| Vet + head keeper | **Approve the protocol set** and re-approve a document on change (FR-3.9) — bounded, and these documents are maintained for regulatory reasons anyway | — | ≈ 1 h/mo | ≈ 1 h/mo | Approval state per document |
...
| S1 protocol answers | Two quarters after Phase 2 | False refusals above 5%, or fewer questions asked than the folder gets opened — a tool nobody trusts is worse than a folder |
| S1 feed forecast | End of season 2 | Not beating the trailing four-week average on MAPE. The average stays either way, so cutting costs nothing |
| S7 investment evaluation | After two full years of evaluations | Every investment returned *cannot attribute* — the estate's investment cadence is too clustered for any method, and the useful residue is the advice to stagger openings, which needs no software |
...
| `agent:ops-copilot` | Two quarters after Phase 2 | Task success below 70%, proposals rejected more often than accepted, **or tasks per week falling toward zero** — the last one is the condition the other two cannot see |
