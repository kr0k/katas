...
The [driving characteristics](../requirements/04-non-functional-requirements.md#driving-characteristics-the-ones-that-shape-the-architecture) say what matters and the [conflict table](../requirements/04-non-functional-requirements.md#where-the-characteristics-conflict-and-what-we-paid) says who wins when two of them collide. This document is the evaluation itself: the quality attributes refined into **scenarios with response measures**, the **architectural styles** we did not choose, and the **sensitivity points, trade-off points, risks and non-risks** the analysis produced.

**What this is and is not.** A real ATAM is a facilitated workshop: stakeholders present, scenarios brainstormed and voted, two evaluation phases. We have no stakeholders in the room — the brief says so — so this is the *analysis* half done honestly and the *elicitation* half declared as an assumption: the scenarios below come from the brief, the goals and the risks, and their priorities are ours to defend, not a stakeholder vote to cite. Every response measure is an existing NFR, game day or eval gate, so nothing here is a new promise; if a row has no verifier, that is the finding.

## Utility tree

Priority is `(importance to the estate, difficulty for us)` on H/M/L. The rows that matter are `(H, H)` and `(H, M)` — those are where the architecture had to actually decide something.
...
Scored against the four driving characteristics plus operability, which for a team of five is a characteristic in everything but name (NFR-OPS-1).
...
| **Space-based (in-memory data grid, replicated processing units)** | Good against *node* loss, irrelevant against *uplink* loss — which is our actual failure mode | Neutral | Neutral | Poor — a grid to operate and reason about | **Rejected as a solution to a problem we do not have**: the estate peaks at ≈ 19 msg/s and ≈ 2,940 gate scans in the peak hour. We are not throughput-bound, we are connectivity-bound |
...
The chosen style is a **hybrid, not a pure one**: edge-first for the estate, event-driven between contexts, a modular monolith as the deployment unit, and separate deployables only where the runtime differs (GPU, edge, ingestion). Each half is justified where it is decided — [ADR-0001](../adrs/ADR-0001-edge-first-store-and-forward.md), [ADR-0004](../adrs/ADR-0004-event-driven-backbone.md), [ADR-0006](../adrs/ADR-0006-edge-vs-cloud-inference.md).

## Sensitivity points

One parameter, one attribute that moves sharply with it. These are the numbers to watch, and each has an owner who watches it.
...
| SP-5 | Companion adoption rate | Growth **and** generative cost | The same number drives the flywheel in [08 §3](../requirements/08-business-case.md#3-the-membership-flywheel) and the token bill. Success on one is spend on the other — the only parameter in the model that is good news twice and a budget alert once |
...
One decision, two attributes moving in opposite directions. These are the rows a reviewer should argue with, because each one is a judgement rather than an arithmetic.
...
**Risks the evaluation surfaced** — each already carried in [requirements/07](../requirements/07-risks-and-mitigations.md) or in TODOS, with an owner:
...
**Non-risks** — analysed and deliberately closed, so they do not need mitigating:
...
- **AI in a safety-critical path.** Structurally impossible rather than policed: tier-0 rules run on the broker with no model in the path, and a model advisory uses a different channel (FR-3.6, FR-3.7).
