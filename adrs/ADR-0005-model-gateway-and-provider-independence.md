...
3. **Bundles are versioned artifacts** (model reference, prompt template, parameters, schema, eval score, owner) declared in Git and promoted through `candidate → shadow → production`. Changing a model is a pull request that must pass the capability's eval suite (ADR-0008). **A swap is not a configuration flip:** models from different providers respond differently to the same prompt, so the bundle carries a **provider-specific prompt variant** and the calibration fitted for that model, and the capability's eval suite is re-run per provider before promotion. That work is what the ≤ 1 day in NFR-EVO-1 covers. What provider independence buys is the absence of *code* coupling — no service changes, no schema changes, no consumer changes — not the absence of validation.
