...
| | 1.2 Returning share of identified households — pass or opt-in account — with ≥ 2 visits in 12 mo¹ | ~10% | 12% / 25% | 40% | S4 |
| | 1.3 Family and season passes as share of admissions (visitor-days admitted on passes ÷ all admissions) | ~20% | 29% / 40% | ≥ 50% — model: ≈ 67% under the lower-bound assumption of [08 §2](08-business-case.md#2-where-the-growth-comes-from) | S4, S5 |
| | 1.4 Revenue per identified household per year (admission + on-site)²; second line: revenue per visitor-day | baseline | ≥ baseline | +30% | S4, S5 |
...
¹ Measured per credential and account from the ticketing platform's export and `GateEntered`; anonymous day tickets are covered by a quarterly exit survey and labelled as an estimate (A14). The growth model works in households; persons only via the party size of 3.5 ([08 §0](08-business-case.md#0-how-to-read-the-numbers)).
² Cash revenue from `TicketPurchased` + `PurchaseRecorded` (FR-2.6) ÷ identified households. Admissions never appear as purchases — there is no admission category — so nothing is double-counted. **OKR = year, guardrail = week:** the S5 experiment cannot wait for an annual figure and uses its own weekly operational guardrail — contribution per visitor-day on discounted blocks not below the 0% blocks − 10% ([thresholds table](../hld/ai-platform/README.md#thresholds-and-cadences-source-of-truth)).
...
⁴ Measured from `PassRenewed`, which exists from Phase 0: the first cohort's renewal is measurable at month 12 and targeted from Phase 3. Zoo-membership renewal norms are 50–70% ([PassPlay 2026](https://passplay.io/blog/posts/what-museum-and-zoo-members-want-from-membership-in-2026)), so 70% is labelled ambitious.
