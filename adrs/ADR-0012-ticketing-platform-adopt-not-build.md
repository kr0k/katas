...
**Serves:** FR-1.1, FR-1.2, FR-1.3, FR-1.4, FR-1.5, FR-1.6, NFR-AVL-1, NFR-AVL-2, NFR-OPS-1, NFR-SEC-2, R9
...
## Decision
1. **Adopt an attraction-ticketing platform (SaaS)** for catalogue, checkout and payments, single and family/season passes, opt-in visitor accounts, gate credentials and sales reconciliation. The Ticketing & Access context in our monolith becomes an **anti-corruption layer**: an adapter that turns vendor webhooks into our events (`TicketPurchased`, `GateEntered`, `GateExited`, `PassRenewed`) and our commands (`PriceUpdated`, GDPR erasure) into vendor API calls. Domain events, read models and the event backbone stay ours ([ADR-0004](ADR-0004-event-driven-backbone.md)).
