...
8. **Kill switch & deterministic fallback** for every capability, with the procedure around it in the [AI incident runbook](../appendix/ai-incident-runbook.md) — fall back first, diagnose second, because the fallback is what the process ran on before the model existed — tested in game days GD-9 and GD-10 of the [resilience validation catalogue](../hld/core/resilience-validation.md) — the same catalogue that verifies the non-AI foundation.
