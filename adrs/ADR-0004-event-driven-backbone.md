...
8. **Consumer-side idempotency is an inbox**, symmetric to the outbox. A consumer applies an event's effect and records `(consumer_id, event_id)` as handled in **one local transaction**; effects that cannot join it (vendor API, e-mail, push) carry an idempotency key derived from the same `event_id`. The CI fitness function that forbids cross-module schema access also fails a subscription handler writing without its inbox row. Per-consumer keys, windows and the bounded failure this admits: [hld/core → consumer-side idempotency](../hld/core/README.md#consumer-side-idempotency-the-inbox).
