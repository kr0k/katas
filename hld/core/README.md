...
| **Ticketing platform (SaaS)** | External: catalogue, checkout and payments, passes, credentials, opt-in accounts, timed-entry slots and daily cap, same-day upgrade credit, sales reconciliation | Vendor | [ADR-0012](../../adrs/ADR-0012-ticketing-platform-adopt-not-build.md); PCI scope stays here |
...
**On-site spend (FR-2.6).** The POS — the ticketing platform's own or a separate system (A13) — posts each transaction as a **signed webhook** (HMAC or mutual TLS); the layer verifies the signature, drops replays on transaction id + timestamp, runs a **runtime value deny-list** (PAN/Luhn, e-mail, phone) over every field and **rejects the event with an incident** if anything matches (the schema-registry CI check cannot see runtime values), maps `pos_terminal_id` to nothing — the zone mapping is Park Operations' — and writes `PurchaseRecorded` to the outbox: transaction id, type (sale / refund / void / correction), net + tax + currency, category without an admission category, terminal, time, and the pseudonymous `visitor_id` only under the purchase-history consent ([ADR-0009](../../adrs/ADR-0009-visitor-privacy-anonymous-counting.md) §7). An unknown terminal lands in an **"unmapped" bucket** and alerts rather than being dropped. Every night the day's `PurchaseRecorded` totals are **reconciled against the vendor's end-of-day totals**; a difference above 1% marks the day's spend *provisional* on every read model and produces an exceptions report for finance. Rate limit and a dead-letter quota protect the outbox from a misbehaving vendor.

**Timed entry and daily cap (FR-1.7).** The default cap is GitOps policy config under the management role; a day-level change by the ops manager — typically proposed by the daily report from the S3 forecast — carries a reason code, is applied through the vendor's cap API, and is published as `CapacityCapChanged` (who / when / why / old / new). The cap applies to **unsold tickets only**; gate validation is untouched. Concurrent edits are **last-write-wins** with both events published and a UI warning; the vendor API returns the **effective cap and the sold count**, and *sold > cap* is an alert (oversell), not a gate refusal.

**Same-day upgrade credit (P-I7).** A day-ticket holder can turn today's ticket into a season pass with the ticket price credited — **once per ticket id, only for a ticket valid today, voided if the ticket is refunded, credit valid 7 days**. The rule is the vendor's to execute; the prompt is ours: the companion shows it only to day-ticket holders without a pass (from the pass state it already fetches), with the ticket id as idempotency key; offline it says "available at the exit and at the gate POS"; the same offer appears at the gate POS and in a next-day nudge. A pass keeps one price for everyone — the credit is not a personal price.

## Estate daily report

The one surface where all five scenarios meet the person who pays. **FR-2.7**: at 21:00 the Countess receives one phone screen of "how was today", built by a **read model in Park Operations** over facts and human decisions already on the backbone — `GateEntered` (with `persons_admitted`), `TicketPurchased`, `PassRenewed`, `PurchaseRecorded`, `CapacityCapChanged`, `ReviewDecided`, `TreatmentStarted` / `TreatmentClosed`, `QueueLengthUpdated`, plus the S3 forecast and `StaffingPlanApproved` from inside the module. It publishes no event and is **not an AI scenario**: the numbers are inserted verbatim, the recommendation is chosen by a fixed rule set, and the optional phrasing reuses the S1 daily-summary drafter as the capability `summarise-estate-day` — same verbatim-numbers rule as [ADR-0010](../../adrs/ADR-0010-grounded-llm-with-guardrails.md) §2, same human approval. Promised in [requirements/01](../../requirements/01-business-goals-and-drivers.md); a `/plan-design-review` of this specification is a TODO before the Phase 1 build ([TODOS.md](../../TODOS.md)).
...
| — | **Footer = freshness** | snapshot id · event lag at 21:00 · provisional flags · template version · phrasing model version or "template" | read model |

**States and rules**

| State | When | What the Countess sees |
| --- | --- | --- |
| SUCCESS | Read model complete at 21:00, verbatim check passed, ops manager approved phrasing before 21:00 | Full report, phrased |
| SUCCESS (template) | No approval by 21:00, or phrasing unavailable | Full report, template wording; approve after 21:00 is logged as "sent as template" and no phrasing is sent later |
| PARTIAL (provisional) | Read-model lag > 15 min at 21:00, or the day's spend reconciliation failed | Report marked *provisional* by symbol **and** word (never colour alone) in the header and footer; **re-issued at 07:00** |
...
| Dead-letter queue by reason | Any PII rejection; > 0.1% of the day for other reasons | PII → incident and vendor ticket; others → fix, replay from the DLQ |
...
- **Unit:** the deny-list (Luhn, e-mail, phone) on generated payloads; terminal → zone mapping; the verbatim checker (every read-model number present, no other number in the text); the recommendation rule set; the days-until-parking-binds model against hand-computed cases.
- **Integration, in the vendor sandbox** ([ADR-0012](../../adrs/ADR-0012-ticketing-platform-adopt-not-build.md) criterion): webhook signature and replay rejection; end-of-day reconciliation with an injected 1% gap; cap API — effective cap, sold count, oversell alert; the upgrade credit end to end.
- **Consumer-driven contract test** for the report's subscriptions: the fields it reads from `TicketPurchased`, `PassRenewed`, `PurchaseRecorded`, `CapacityCapChanged`, `ReviewDecided` and `TreatmentStarted/Closed` are pinned; a producer change that breaks them fails the producer's build.
- **Property tests:** P-I7 — double upgrade, refund after upgrade, expired credit, upgrade of a ticket for another day — violations = 0; the S5 policy engine's contribution constraint with the A15 parameters as generated inputs ([S5](../scenarios/dynamic-family-passes/README.md)).
- **End-to-end report day with an injectable clock:** happy path; closed day; first season; approve after 21:00; uplink down at 20:50 (GD-15).
...
- **Game days** GD-14 (POS sends a card number) and GD-15 (uplink down on report day) in the [catalogue](resilience-validation.md).
...
- **Resilience validation:** fifteen scripted game days with expected behaviour, metric, cadence and owner → [resilience-validation.md](resilience-validation.md).
