...
Seven AI scenarios, two radio technologies and an edge tier are a lot for ≤ 5 engineers (NFR-OPS-1, R9), so the scope is sequenced rather than cut: each phase has an entry gate, and no model is promoted before the data it needs exists (A6, R7).

| Phase | Enter when | Foundation delivered | AI delivered | Verified by |
| --- | --- | --- | --- | --- |
| **0 · Foundation** | Day 1 | Edge tier: MQTT broker, LoRaWAN gateway, gate readers, tier-0 safety rules, staff devices. Ticketing platform adopted and integrated (ADR-0012). Event backbone, business monolith skeleton, data platform, GitOps. **Investment register** (FR-2.9) — a form, and the prerequisite for S7 a year later. | None. Data capture only — every sensor and gate produces events from day one. | Game days: uplink loss, broker failover, gate offline, revocation after outage, safety alert to a human → [resilience validation](hld/core/resilience-validation.md) |
...
| S6 | [Content drafting](hld/scenarios/content-drafting/README.md) | Statistical candidate finding + grounded LLM | Attendance — the one lever the platform had disclaimed | 1.7 | Yes — a human publishes; there is no automated publish path |
| S7 | [Investment evaluation](hld/scenarios/investment-evaluation/README.md) | Causal inference (difference-in-differences, synthetic control) | None directly — it tells the Countess where the *next* money should go | 2.5 | Management reads the verdict, including "cannot attribute" |
...
| Say whether an investment worked, or that it cannot be told | FR-2.9, FR-2.10 | [S7](hld/scenarios/investment-evaluation/README.md) | [ADR-0008](adrs/ADR-0008-ai-evaluation-and-production-monitoring.md), [ADR-0004](adrs/ADR-0004-event-driven-backbone.md) |
| Answer a keeper's procedural question, and forecast feed | FR-3.9, FR-3.10 | [S1](hld/scenarios/animal-welfare-monitoring/README.md) | [ADR-0006](adrs/ADR-0006-edge-vs-cloud-inference.md), [ADR-0007](adrs/ADR-0007-human-in-the-loop-confidence-bands.md), [ADR-0008](adrs/ADR-0008-ai-evaluation-and-production-monitoring.md), [ADR-0010](adrs/ADR-0010-grounded-llm-with-guardrails.md), [ADR-0013](adrs/ADR-0013-role-agents-on-typed-tools.md) |
...
| Draft content from what the cameras already noticed | FR-4.6 | [S6](hld/scenarios/content-drafting/README.md) | [ADR-0006](adrs/ADR-0006-edge-vs-cloud-inference.md), [ADR-0007](adrs/ADR-0007-human-in-the-loop-confidence-bands.md), [ADR-0008](adrs/ADR-0008-ai-evaluation-and-production-monitoring.md), [ADR-0009](adrs/ADR-0009-visitor-privacy-anonymous-counting.md) |
| Monitor the plant collection on the welfare pipeline | FR-3.8 | [S1](hld/scenarios/animal-welfare-monitoring/README.md) | [ADR-0006](adrs/ADR-0006-edge-vs-cloud-inference.md), [ADR-0007](adrs/ADR-0007-human-in-the-loop-confidence-bands.md), [ADR-0008](adrs/ADR-0008-ai-evaluation-and-production-monitoring.md) |
