...
| FR-2.9 | **Investment register**: every capital investment above a threshold is recorded before the work starts — what, where, when it opened, what it cost, and which zones it touches | MUST |
| FR-2.10 🤖 | **Did the investment work?** A causal effect estimate per investment on the zones it touched, with an interval and an explicit *cannot attribute* verdict when the period is confounded → [S7](../hld/scenarios/investment-evaluation/README.md) | SHOULD |
...
| FR-3.9 🤖 | **Protocol answers for staff**: a keeper or vet asks a procedural question and gets the answer from the estate's approved protocol set, with the actionable steps inserted verbatim and the document cited — or a refusal and a named person to call when no approved document covers it → [S1](../hld/scenarios/animal-welfare-monitoring/README.md) | SHOULD |
| FR-3.10 🤖 | **Feed consumption forecast**: per store and species, from recorded feeding and scale deltas, as an ordering proposal a keeper approves → [S1](../hld/scenarios/animal-welfare-monitoring/README.md) | COULD |
