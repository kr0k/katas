...
| Knowledge base | Structured facts + curated narrative; the only citable source. **Safety, allergen and price fields exist per language, each human-approved** — the model composes around them and may not translate them (NFR-LNG-1) | No |
...
**Cost budget by request class.** The split that drives latency drives cost too: planning is a large-tier call, an FAQ answer a small-tier one, and half of those never reach a model. Day planning and re-planning therefore carry ≈ 90% of the generative bill, and the scenario as a whole costs ≈ **€0.04 per companion household visit** — see [what the generative capabilities cost](../../ai-platform/README.md#what-the-generative-capabilities-cost).
