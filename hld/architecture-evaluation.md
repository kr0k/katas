...
| **Evolvability of the AI layer** | Model swap | A better model appears for a capability | Registry PR + eval gate; ≤ 1 day *including* a provider-specific prompt variant and a re-run eval suite; zero business-service changes | (H, M) | NFR-EVO-1, OKR 5.2 |
...
| | New capability | An eighth AI use case is added | A new consumer on the backbone; no producer changes; no shared schema | (M, L) | [ADR-0004](../adrs/ADR-0004-event-driven-backbone.md) §3 |
...
| | Agency | A poisoned knowledge-base document tells an agent to issue a free pass | Content is quoted, never obeyed; no tool exists to grant it; the attempt is a tool-error span and is attributable in the page history | (H, M) | [GD-17](core/resilience-validation.md), [ADR-0013](../adrs/ADR-0013-role-agents-on-typed-tools.md), R21 |
| | Agency | An agent degrades quietly — same model, worse wiring | Task success and tool-error rate on a weekly labelled sample; the **agent** version rolls back without touching the model bundle | (H, M) | OKR 5.4, [agents](ai-platform/agents.md#in-production-evaluation-and-rollback-of-the-agent-rather-than-the-model) |
| | Withdrawal | The whole agentic layer is switched off | All six processes still complete: every read is also a dashboard, every draft is also a form | (H, L) | GD-19, [process table](../README.md#where-ai-sits-in-the-working-day) |
...
| SP-2 | The assumed cost-of-error ratio per capability | Safety **and** the vet's hours | Bands are derived from the [cost-of-error table](../adrs/ADR-0007-human-in-the-loop-confidence-bands.md#what-an-error-costs-and-what-that-sets), so the sensitive parameter is the ratio rather than the number 0.9. Argue welfare down to 5:1 and the medium band narrows until the weekly audit of unflagged hours is the only thing catching misses; argue it up to 50:1 and the queue outgrows the vet hours in the [human roles table](ai-platform/README.md#humans-in-the-loop-who-does-what), which is the real constraint either way |
| SP-3 | Cacheable share of the session prefix | Cost-efficiency | Prompt caching is ≈ half the generative bill: €34,600 with it, €68,000 without. A prompt refactor that breaks prefix stability is a cost incident |
...
| SP-8 | Step and token budget per agent task | Cost **and** honesty of the answer | The budget is what turns an unanswerable question into "I cannot answer this from defined metrics" instead of a fabricated number (GD-20). Double it and cost per task doubles with little gain; halve it and useful multi-step tasks start failing — the copilot's whole value is three or four lookups in one go |
| SP-9 | Share of agent tool calls that are effectful | Auditability **and** review load | Today one tool in six has an effect and each needs a human to commit it. Raise that share and the approval surfaces become the bottleneck; the sharper edge is `open_review`, which creates work rather than action and can flood the vet's queue without ever changing anything |
...
| TP-5 | Adopt the ticketing platform | Cost, operability, PCI scope out of our estate | Control: 16 must-have criteria become a procurement dependency, and a missing one changes Phase 0 scope — the three credential criteria are the newest and the most likely to be missing | [ADR-0012](../adrs/ADR-0012-ticketing-platform-adopt-not-build.md), [TODOS.md](../TODOS.md) |
...
| TP-10 | One metric layer as the single source for every quoted number | Consistency — the daily report, the dashboards, `ask-the-estate` and the pricing and forecasting models can no longer compute "attendance" two ways; and a natural-language question resolves to a defined metric or is refused | Independent verification. Before, a wrong formula was wrong in one place and the disagreement was the alarm; now every consumer inherits the same error **and agrees while doing it**, which is harder to notice than a contradiction. What we keep is attribution: one owner, one formula, one Git history, and the definition travelling with the value wherever it is quoted | [hld/core → data platform](core/README.md#data-platform), [metric-layer gate](ai-platform/README.md#thresholds-and-cadences-source-of-truth) |
| TP-8 | Giving agents tools rather than keeping them assistants | AI reaches inside multi-step decisions — the value the "embedded in the process" claim rests on | A new attack surface (injection, loops, retries) and statistical rather than exact regression testing; four game days exist only for this | [ADR-0013](../adrs/ADR-0013-role-agents-on-typed-tools.md), GD-17–GD-20 |
| TP-9 | Agents propose, humans commit | Accountability stays with a person, and no second place can authorise an action | Latency: a proposal waits for an approval cycle, so the copilot cannot be the fast path for anything. We accept a slower loop over an unreviewed one | [ADR-0013](../adrs/ADR-0013-role-agents-on-typed-tools.md) §3, [ADR-0007](../adrs/ADR-0007-human-in-the-loop-confidence-bands.md) |
