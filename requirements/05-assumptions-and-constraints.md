...
| A16 | **Ride entitlements imply a revenue model we do not currently assume.** Admission plus on-site spend is what the [business case](../appendix/business-case-model.md) prices; pay-per-ride and premium experiences are not in it. If the Countess wants credits on the pass card ([ADR-0011](../adrs/ADR-0011-offline-ticket-validation.md) §7), that is a pricing decision she owns, and it adds a revenue line to the model rather than a feature to the platform. | Without the decision, the card carries entry and cashless only, and the architecture is unchanged — the entitlement counter simply stays at zero. |
