...
- **Planned spend ≈ €34,300 a year at 15,000 visitors/day**, ≈ €0.04 per companion household visit against ≈ €21 of gross revenue per visitor-day. That is inside the €50k line in the [cost model](../../requirements/04-non-functional-requirements.md#cost-model-tco-50) and ≈ 0.04% of revenue against NFR-COST-1's 2% ceiling.
- **Day planning and re-planning are ≈ 90% of it**, which is why routing is tiered and a re-plan touches only the affected stops.
- **The FAQ cache and prompt caching are a design condition, not an optimisation.** Without them the same traffic costs ≈ €67,100 — outside the line. Same shape as the [ingestion counterfactual](../core/edge-and-connectivity.md#capacity-check-at-15000-visitorsday-by-traffic-class).
- **A peak day is 2.0× an average one**, not an order of magnitude, because load is bounded by households present rather than by concurrency. The budget is therefore seasonal, not a flat twelfth.
